-- =============================================================================
--  ps-mdt :: ESX (es_extended) database schema
--  Generated from qbcore.sql. Identical MDT tables, but the QBCore-only
--  `ALTER TABLE player_vehicles ...` block is replaced by an ESX compatibility
--  layer (see bottom: `players` / `player_vehicles` views over users/owned_vehicles).
--  Import this file (NOT qbcore.sql / qbx.sql) on ESX servers.
-- =============================================================================
...
-- =============================================================================
--  ESX COMPATIBILITY LAYER for ps-mdt
--  Target: ESX Legacy (1.9+). Requires MariaDB (uses ADD COLUMN IF NOT EXISTS).
--
--  ps-mdt was written against the QBCore/Qbox schema and queries two framework
--  tables directly: `players` and `player_vehicles`. ESX uses `users` and
--  `owned_vehicles`, with a different shape. Rather than rewrite every query,
--  we:
--    1) ensure the ESX base tables have the columns the MDT needs, then
--    2) expose QBCore-shaped VIEWS named `players` and `player_vehicles`.
--  Every `mdt_*` table above is framework-independent and used unchanged.
--
--  NOTE: vehicle display names depend on how your server stores the model in
--  `owned_vehicles.vehicle` (`$.model`). If it stores a model HASH the MDT will
--  show that value; if it stores a spawn name it resolves cleanly.
-- =============================================================================

-- 1) Ensure ESX `users` has the columns the `players` view references.
--    `metadata` exists on ESX Legacy; `phone_number` exists on most builds.
--    Added defensively so the view never references a missing column.
ALTER TABLE `users`
...
-- Normalise empty-string metadata to NULL so the `players` view's JSON_EXTRACT
-- calls never hit invalid JSON (JSON_EXTRACT('') raises an error; NULL is safe).
UPDATE `users` SET `metadata` = NULL WHERE `metadata` = '';

-- 2a) Ensure `owned_vehicles` has a surrogate id (mdt_impound.vehicleid
--     references player_vehicles.id). AUTO_INCREMENT needs an index, so it is
--     added as a UNIQUE key (kept separate from any existing PRIMARY KEY).
ALTER TABLE `owned_vehicles`
  ADD COLUMN IF NOT EXISTS `id` INT(11) NOT NULL AUTO_INCREMENT UNIQUE KEY FIRST;

-- 2b) MDT vehicle columns + the MDT impound flag. The MDT writes to these
--     through the `player_vehicles` view, so they must be REAL columns.
ALTER TABLE `owned_vehicles`
  ...
  ADD COLUMN IF NOT EXISTS `mdt_impound_state` tinyint(1) NOT NULL DEFAULT 1;

-- 3) QBCore-shaped `players` view over ESX `users`.
--    Exposes citizenid/charinfo/metadata/job in the JSON shape the MDT expects.
--    The only column the MDT ever writes is `metadata` (maps 1:1 to users).
...
SELECT
  ...
  JSON_OBJECT(
    ...
    'gender',      CASE WHEN LOWER(COALESCE(u.sex, '')) IN ('f', 'female', 'w', '1') THEN 1 ELSE 0 END,
  ...
  JSON_OBJECT(
    ...
    'type',  CASE
               WHEN u.job IN ('police','lspd','bcso','sahp','sast','sheriff','statepolice','fib','gov') THEN 'leo'
               WHEN u.job IN ('ambulance','ems','lsfd','fire','doctor') THEN 'ems'
               WHEN u.job IN ('lawyer','judge','attorney') THEN 'doj'
               ELSE NULL
             END,
    'grade', JSON_OBJECT('name', COALESCE(jg.label, CAST(u.job_grade AS CHAR)), 'level', u.job_grade),
    'onduty', true
  ) AS job
FROM `users` u
...
-- 4) QBCore-shaped `player_vehicles` view over ESX `owned_vehicles`.
--    `vehicle` is surfaced as the model from the stored properties JSON.
--    `state` mirrors the MDT impound flag (1 = released, 2 = impounded) and is a
--    real column so the MDT's impound/release UPDATEs succeed through the view.
CREATE OR REPLACE VIEW `player_vehicles` AS
SELECT
  ov.id                                                       AS id,
  ov.owner                                                    AS citizenid,
  ov.plate                                                    AS plate,
  CASE WHEN JSON_VALID(ov.vehicle) THEN JSON_UNQUOTE(JSON_EXTRACT(ov.vehicle, '$.model')) ELSE NULL END          AS vehicle,
  CASE WHEN JSON_VALID(ov.vehicle) THEN COALESCE(JSON_EXTRACT(ov.vehicle, '$.fuelLevel'), 100) ELSE 100 END       AS fuel,
  CASE WHEN JSON_VALID(ov.vehicle) THEN COALESCE(JSON_EXTRACT(ov.vehicle, '$.engineHealth'), 1000) ELSE 1000 END  AS engine,
  CASE WHEN JSON_VALID(ov.vehicle) THEN COALESCE(JSON_EXTRACT(ov.vehicle, '$.bodyHealth'), 1000) ELSE 1000 END    AS body,
  ov.mdt_impound_state                                        AS state,
  ov.mdt_vehicle_information                                  AS mdt_vehicle_information,
  ov.mdt_vehicle_points                                       AS mdt_vehicle_points,
  ov.mdt_vehicle_status                                       AS mdt_vehicle_status,
  ov.mdt_vehicle_stolen                                       AS mdt_vehicle_stolen,
  ov.mdt_vehicle_boloactive                                   AS mdt_vehicle_boloactive,
  ov.mdt_vehicle_image                                        AS mdt_vehicle_image
