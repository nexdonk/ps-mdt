...
function EnsureProfileExists(citizenid)
    ...
    -- Try online player first (framework-agnostic; QBCore/Qbox return a
    -- .PlayerData wrapper, ESX returns the xPlayer object, so resolve via the
    -- ps_lib bridge instead of assuming a shape).
    local src = ps.getSource and ps.getSource(citizenid) or nil
    if src then
        ...
        if fullname and fullname ~= '' and fullname ~= 'Unknown' then
...
function getCitizens(source)
    if not IsPoliceJob(ps.getJobName(source), ps.getJobType(source)) then
        return {}
    end
