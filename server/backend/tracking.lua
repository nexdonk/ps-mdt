...
-- Acquire QBCore only if it is actually running. NOTE: `if exports['qb-core']`
-- is always truthy (it's a proxy), so calling :GetCoreObject() on a non-QB
-- server throws. Gate on GetResourceState and pcall instead.
local QBCore
...
local function getVehicleTrackers()
    ...
    -- Framework-agnostic fallback (ESX / any ps_lib framework).
    if ps and ps.getAllPlayers then
        local players = ps.getAllPlayers() or {}
        for _, playerId in pairs(players) do
            if ps.getJobDuty and ps.getJobDuty(playerId) then
